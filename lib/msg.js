...
const sms = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        // Safely access contextInfo, avoid undefined errors
        ...
        if (m.quoted) {
            ...
            if (quoted.viewOnceMessageV2) {
                ...
                m.quoted.mtype = type
                m.quoted.id = m.msg.contextInfo ? m.msg.contextInfo.stanzaId : undefined
                m.quoted.chat = m.msg.contextInfo ? (m.msg.contextInfo.remoteJid || m.chat) : m.chat
                m.quoted.isBot = m.quoted.id ? m.quoted.id.startsWith('BAES') && m.quoted.id.length === 16 : false
                m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('BAE5') && m.quoted.id.length === 16 : false
                m.quoted.sender = m.msg.contextInfo ? conn.decodeJid(m.msg.contextInfo.participant) : undefined
